    ...
    async function start() {
      ...
      try {
        ...
        if (!cancelled) {
          setStatus("error");
          setErrorMessage(
            "مش قادرين نوصل للمايك. اسمح للموقع باستخدام الميكروفون من إعدادات المتصفح وجرّب تاني."
          );
        }
        return;
      }
      ...
      try {
        ...
        if (!cancelled) {
          setStatus("error");
          setErrorMessage("مش قادرين نجهّز الصوت في المتصفح. جرّب متصفح تاني أو حدّثه.");
        }
      }
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 py-6">
      <div className="flex max-h-[88vh] w-full max-w-[520px] flex-col overflow-hidden rounded-[22px] bg-white shadow-[0_18px_60px_rgba(0,0,0,0.25)]">
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          ...
          >
            {isLive && (
              <span className="mr-1 inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-[#1AA681] align-middle" />
            )}
        ...
        {status === "error" && errorMessage && (
          <div className="mx-6 mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-right text-[13px] leading-6 text-red-700">
            {errorMessage}
          </div>
        )}
          ...
          <div className="flex flex-col gap-3">
            {lines.length === 0 && status !== "error" && (
              <p className="py-8 text-center text-[13px] leading-7 text-[#8FA0A5]">
                {isLive
                  ? "اتكلم عادي — نجدة بيسمعك وهيرد عليك بصوته."
                  : "بنجهّز المكالمة…"}
              </p>
            )}
            ...
            {lines.map((line) =>
              line.role === "user" ? (
                <div key={line.id} className="flex justify-end">
                  <p
                    dir="auto"
                    className="max-w-[80%] rounded-[16px] rounded-tr-[5px] bg-[#E0F5EF] px-4 py-2.5 text-right text-[13px] leading-6 text-[#17333A]"
                  >
                    {line.text}
                  </p>
                </div>
                ...
                <div key={line.id} className="flex justify-start">
                  <p
                    dir="auto"
                    className="max-w-[80%] rounded-[16px] rounded-tl-[5px] border border-gray-100 bg-white px-4 py-2.5 text-right text-[13px] leading-6 text-[#263B42] shadow-[0_3px_14px_rgba(0,0,0,0.04)]"
                  >
                    {line.text}
                  </p>
                </div>
        ...
        <div className="border-t border-gray-100 px-6 py-4">
          ...
          <p className="mt-3 text-center text-[11px] leading-5 text-[#8FA0A5]">
            نجدة مش دكتور ومش بيشخّص — لو الأعراض شديدة اتصل بالإسعاف 123.
          </p>
        </div>
      </div>
    </div>
